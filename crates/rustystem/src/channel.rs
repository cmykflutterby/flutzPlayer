...
impl Channel {
    ...
    pub(crate) fn set_percussion_channel(&mut self, is_percussion_channel: bool) {
        if self.is_percussion_channel == is_percussion_channel {
            return;
        }
        self.is_percussion_channel = is_percussion_channel;
        self.bank_number = if is_percussion_channel { 128 } else { 0 };
    }
