...
impl InstrumentRegion {
    fn new(
        ...
        // Some fonts carry malformed loop offsets for one-shot regions.
        // If loop mode is enabled but loop points are not usable inside the region span,
        // downgrade this region to no-loop so loading can continue safely.
        let start = sample.start
            + 32768 * gs[GeneratorType::START_ADDRESS_COARSE_OFFSET as usize] as i32
            + gs[GeneratorType::START_ADDRESS_OFFSET as usize] as i32;
        let end = sample.end
            + 32768 * gs[GeneratorType::END_ADDRESS_COARSE_OFFSET as usize] as i32
            + gs[GeneratorType::END_ADDRESS_OFFSET as usize] as i32;
        let start_loop = sample.start_loop
            + 32768 * gs[GeneratorType::START_LOOP_ADDRESS_COARSE_OFFSET as usize] as i32
            + gs[GeneratorType::START_LOOP_ADDRESS_OFFSET as usize] as i32;
        let end_loop = sample.end_loop
            + 32768 * gs[GeneratorType::END_LOOP_ADDRESS_COARSE_OFFSET as usize] as i32
            + gs[GeneratorType::END_LOOP_ADDRESS_OFFSET as usize] as i32;

        let loop_mode = LoopMode::from_i16(gs[GeneratorType::SAMPLE_MODES as usize]);
        if loop_mode != LoopMode::NoLoop
            && (start_loop < start || end_loop > end || end_loop <= start_loop)
        {
            gs[GeneratorType::SAMPLE_MODES as usize] = 0;
        }
